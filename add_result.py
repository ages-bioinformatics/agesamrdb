...
# CLI definitions
parser = argparse.ArgumentParser(description="agres - helperscript to add single result to database - methods allowed are: resfinder, bakta, isescan")
# db definitions
parser.add_argument('-d','--database',dest='database', help="mysql database name or path to sqlite-db [./agres.db]", default="./agres.db")
parser.add_argument('-H','--hostname',dest='hostname', help="mysql hostname; if not provided sqlite-db will be tried", required=False)
...
parser.add_argument('--mode', dest='mode', help='define which input type was used (fasta/fastq) - only in use for resfinder [fasta]', default='fasta')

# optional reference to external id or provide a sample_name or both
parser.add_argument('--external_id', dest='external_id', help="optional external id for other db", required=False, metavar="INT")
parser.add_argument('--sample_name', dest='sample_name', help="optional sample name to be displayed", required=False, metavar="NAME")
parser.add_argument('--assembly', dest='assembly', help="imports contig in Fasta-format (dna) to allow visualization (run with resfinder only)", required=False, metavar="FASTA")
...
def main():
    ...
    # interaction with data in database: fetch sample, read results to df and write to database
