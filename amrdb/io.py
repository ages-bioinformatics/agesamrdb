...
def read_isescan_results(input_file: str) -> pd.DataFrame:
    ...
    column_mapping = {"isBegin":"is_start_pos","isEnd":"is_end_pos","ncopy4is":"is_copy_number",
        "start1":"ir_start_pos1","end1":"ir_end_pos1","start2":"ir_start_pos2","end2":"ir_end_pos2",
        "orfBegin":"orf_start_pos","orfEnd":"orf_end_pos","strand":"orientation", "E-value":"e_value",}
...
def read_bakta_results(input_file: str) -> pd.DataFrame:
    ...
    parses tabular result and return df
    """
    header = ["seqID","product_type","ref_pos_start","ref_pos_end","orientation","locus_tag","gene_name","product","db_xref"]
    df = pd.read_csv(input_file, sep="\t", comment="#", names=header)

    return df


def read_resfinder_results(resfinder_dir: str, extract_coordinates: bool=True) -> pd.DataFrame:
    """
