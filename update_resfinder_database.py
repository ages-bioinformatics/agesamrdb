...
def read_resfinder_databases(resfinder_db_dir):
    ...
    for db_file in glob.glob(resfinder_db_dir+"/*.fsa"):
        ...
        for seqrecord in SeqIO.parse(db_file, "fasta"):
            entry = {"name": seqrecord.id, "sequence":str(seqrecord.seq), "crc32_hash":calc_sequence_hash(str(seqrecord.seq))}
...
def identify_columns(records):
    """
    uses records list of dicts to create dataframe and splits name using regex into:
    short_name, main_numbering, subseq_numbering and accession
    subseq_numbering: internally used as allels of z.B. sul1 which are never shown to user (e.g. 1)
    accession: important unique accession of external reference sequence (ncbi, ena) 
    main_numbering: number of resistance gene variant that is displayed to user (e.g. 52B)
    short_name: basic gene name (e.g. blaTEM)
    """    
    
    df = pd.DataFrame.from_records(records)
    df[["short_name","subseq_numbering","accession"]] = df["name"].str.extract("([^_]*)[-_]([^_]*)_([A-Z]+_[A-Z0-9.]*|[A-Z0-9.]*):*.*$", expand=True)

    try:
        df[["short_name", "main_numbering"]] = df["short_name"].str.extract("([^\d^(]*)-*([()0-9A-Za-z'-]*)", expand=True)
    except ValueError:
        df["main_numbering"] = ""
...
def update_existing_sequences(df_new, session):
    ...
    and closest accession (single acn. may have multiple sequences associated then)
    ...
    for i, row in df_new.iterrows():
        ...
        if entry:
            ...
            # i.e. only in case a completely new type of sequence (not variant/allel) is added
...
def write_phenotypes(phenotypes_df, session):
    ...
    for groupname, sub_df in phenotypes_df.groupby(["Phenotype","Class"]):
        linked_sequences = session.query(ResfinderSequence).filter(ResfinderSequence.accession.in_(sub_df["sequence_identifier"].to_list())).all()
        session.add(Phenotype(phenotype=groupname[0], class_name=groupname[1], sequences=linked_sequences))
    session.commit()
